...
// IsvEnterpriseValidator is in charge of starting up the web server and serving our router
type IsvEnterpriseValidator struct {
	ctrl.Manager
	logr.Logger
	runtime.Decoder
}
...
var validator *IsvEnterpriseValidator

// add our validator to a manager
func Add(m ctrl.Manager) error {
	validator = &IsvEnterpriseValidator{
		m,
		m.GetLogger(),
...
func (v *IsvEnterpriseValidator) Start(ctx context.Context) error {
...
func (v *IsvEnterpriseValidator) validateEnterprise(writer http.ResponseWriter, request *http.Request) {
...
func (v *IsvEnterpriseValidator) writeError(writer http.ResponseWriter, err error, msg string, code int) {
...
func isUserAllowed(userInfo authenticationv1.UserInfo) bool {
	...
	return infoArray[0] == "system" &&
		...
		infoArray[3] == "isv-addon-controller-manager"
}
